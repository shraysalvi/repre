...
export const DisplacementSphere = props => {
  ...
  useEffect(() => {
    ...
    camera.current = new PerspectiveCamera(86, innerWidth / innerHeight, 0.1, 100);
    ...
    startTransition(() => {
      geometry.current = new SphereBufferGeometry(32, 100, 800);
  ...
  useEffect(() => {
    ...
    const ambientLight = new AmbientLight(colorWhite, themeId === 'light' ? 0.8 : 0.28);

    dirLight.position.z = 260;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (uniforms.current !== undefined) {
        uniforms.current.time.value = 0.00008 * (Date.now() - start.current);
      }
